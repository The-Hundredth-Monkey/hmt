...
#>   
    ...
    [CmdletBinding()]
    param(
        [Alias('ShowHiddenFiles')]
        [switch]$EnableShowHiddenFiles,
        [Alias('DontShowHiddenFiles')]
        [switch]$DisableShowHiddenFiles,

        [Alias('ShowProtectedOSFile')]
        [switch]$EnableShowProtectedOSFiles,
        [Alias('DontShowProtectedOSFile')]
        [switch]$DisableShowProtectedOSFiles,

        [Alias('ShowFileExtensions')]
        [switch]$EnableShowFileExtensions,
        [Alias('DontShowFileExtensions')]
        [switch]$DisableShowFileExtensions,
        
        [Alias('ShowFullPathInTitleBar')]
        [switch]$EnableShowFullPathInTitleBar,
        [Alias('DontFullPathInTitleBar')]
        [switch]$DisableShowFullPathInTitleBar,

        [Alias('ExpandToOpenFolder')]
        [switch]$EnableExpandToOpenFolder,
        [Alias('DontExpandToOpenFolder')]
        [switch]$DisableExpandToOpenFolder,

        [Alias('OpenToQuickAccess')]
        [switch]$EnableOpenToQuickAccess,
        [Alias('DontOpenToQuickAccess')]
        [switch]$DisableOpenToQuickAccess,

        [Alias('ShowRecentFilesInQuickAccess')]
        [switch]$EnableShowRecentFilesInQuickAccess,
        [Alias('DontShowRecentFilesInQuickAccess')]
        [switch]$DisableShowRecentFilesInQuickAccess,

        [Alias('ShowFrequentFoldersInQuickAccess')]
        [switch]$EnableShowFrequentFoldersInQuickAccess,
        [Alias('DontShowFrequentFoldersInQuickAccess')]
    ...
    if(Test-Path -Path $advancedKey) {
        if($EnableShowHiddenFiles) {Set-ItemProperty $advancedKey Hidden 1}
        if($DisableShowHiddenFiles) {Set-ItemProperty $advancedKey Hidden 0}
        ...
        if($EnableOpenToQuickAccess) {Set-ItemProperty $advancedKey LaunchTo 2}
        if($DisableOpenToQuickAccess) {Set-ItemProperty $advancedKey LaunchTo 1}
    }
